...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const routesUserCanView = routes
    .map((section) => {
      // Filtra apenas os itens que o usuário pode acessar
      const filteredItems = section.items
        .map((item) => {
          // Verifica se o item não possui subItens e é permitido
          const canViewItem =
            (!item.permission || user.permissions.includes(item.permission)) &&
            (!item.subItems || item.subItems.length > 0)

          if (!item.subItems) {
            // Caso o item não tenha subItens, cria um link direto
            return canViewItem ? { ...item, isLink: true } : null
          }

          // Filtra subitens se existirem
          const filteredSubItems = item.subItems?.filter(
            (subItem) =>
              !subItem.permission || user.permissions.includes(subItem.permission)
          ) || []
        ...
        .filter(Boolean) // Remove os itens que ficaram como `null`

      // A seção só é exibida se houver pelo menos um item visível
      const canViewSection =
        (!section.permission || user.permissions.includes(section.permission)) &&
        filteredItems.length > 0

      return canViewSection ? { ...section, items: filteredItems } : null
    })
        ...
        <Link href="/">
          ...
          >
            ...
            <div className="grid flex-1 text-left text-sm leading-tight">
              <span className="truncate font-semibold">Escola START</span>
            </div>
          </SidebarMenuButton>
        </Link>
      </SidebarHeader>
      <SidebarContent>
        <AppSidebarNav sections={routesUserCanView} />
      </SidebarContent>
      <SidebarFooter>
